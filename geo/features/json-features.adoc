...
This resource supports spatial filtering with the link:../../common/parameters/bbox.adoc[`bbox` parameter], temporal filtering with the link:../common/parameters-datetime.adoc[`datetime` parameter]. In addition, the resource may also support more complex filters using the link:../../common/parameters/filter.adoc[`filter` parameter].
...
APIs that support coordinates in coordinate reference systems other than https://en.wikipedia.org/wiki/World_Geodetic_System#WGS84[World Geodetic System 1984] may also support the link:../../common/parameters/crs.adoc[`crs` parameter] to request that the geometries in a representation of the resource are in a specific coordinate reference system.
